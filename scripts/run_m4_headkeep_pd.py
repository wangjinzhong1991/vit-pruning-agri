#!/usr/bin/env python3
"""
run_m4_headkeep_pd.py — PlantDoc 版 headkeep 对照实验
"保留训练完成的分类头、直接剪枝"：baseline FT 完成后保存权重，
再在 FT 好的模型上 prune 25/50/75% head，立即 eval（分类头是训练好的）。
协议与 run_m4_plantdoc.py 完全一致：同权重、同 FT 超参、同 80/20 固定划分(SPLIT_SEED=42)。

输出: results/m4_headkeep_pd/m4_headkeep_pd_seed{seed}.json
     results/m4_headkeep_pd.json  (3-seed mean±std 汇总)
"""
...
LR = 5e-5
WD = 0.01
SPLIT_SEED = 42  # 固定 80/20 划分，与 run_m4_plantdoc.py 一致

PT_WEIGHTS = Path("PRETRAINED_VIT")
DATASET_PATH = Path("DATASETS/plantdoc")

# 与 run_m4_plantdoc.py 完全相同的 transform
...
def finetune(model, train_loader, val_loader, seed, tag, out):
    ...
    for ep in range(EPOCHS):
        ...
        (OUT_DIR / f"m4_headkeep_pd_seed{seed}.json").write_text(json.dumps(out, indent=2))
    return best
...
def run_seed(seed):
    out_path = OUT_DIR / f"m4_headkeep_pd_seed{seed}.json"
    if out_path.exists():
    ...
    # 与 run_m4_plantdoc.py 相同：固定 SPLIT_SEED 的 80/20 random_split
    ...
    print(f"\n=== seed {seed} (headkeep, PlantDoc, {len(train_ds)}/{len(val_ds)}, {num_classes} cls) ===", flush=True)

    # 1) baseline FT（与 m4_pd 相同协议），保存权重
    if "baseline" not in out["results"]:
        ...
        torch.save(m.state_dict(), OUT_DIR / f"m4_headkeep_pd_seed{seed}_baseline_ft.pth")
        (OUT_DIR / f"m4_headkeep_pd_seed{seed}.json").write_text(json.dumps(out, indent=2))
    else:
        m = make_model(num_classes).to(DEVICE)
        m.load_state_dict(torch.load(OUT_DIR / f"m4_headkeep_pd_seed{seed}_baseline_ft.pth", map_location=DEVICE, weights_only=True))
...
if __name__ == "__main__":
    ...
    for k, v in summary.items():
        ...
        v["n"] = len(a)
    (RES / "m4_headkeep_pd.json").write_text(json.dumps(summary, indent=2))
    print("\n=== 汇总 (PlantDoc 保留头 + 直接剪枝) ===")
    for k, v in summary.items():
