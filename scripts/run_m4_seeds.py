#!/usr/bin/env python3
"""
PlantVillage 3-seed unified-protocol experiments (pruning + fine-tuning)
...
- 输出: results/m4/m4_seed{seed}.json（每阶段完成即写盘，可续跑）
"""
...
def make_model():
    ...
    # 剥离 1000 类分类头（本地模型 head 是 38 类）
    sd = {k: v for k, v in sd.items() if not k.startswith("head.")}
    missing, unexpected = m.load_state_dict(sd, strict=False)
    # 允许缺失 head（本地新 head 随机初始化，由 FT 学习）
...
def finetune(model, train_loader, val_loader, seed, tag, out):
    ...
    for ep in range(EPOCHS):
        ...
        (OUT_DIR / f"m4_seed{seed}.json").write_text(json.dumps(out, indent=2))
    return best
...
def run_seed(seed):
    out_path = OUT_DIR / f"m4_seed{seed}.json"
    if out_path.exists():
...
if __name__ == "__main__":
    ...
    for s in SEEDS:
        run_seed(s)
    print(f"\nALL DONE in {(time.time()-t0)/3600:.1f}h", flush=True)
